...
#define sendParams(fd,id,data,len) sendWebData(fd,FCGI_PARAMS,id,data,len)
...
// =========================================================================================================== //
int requestphp(char *data)
{
	int fd;
	fd=createSocket(9000);
	sendGetValue(fd);
	sendBeginRequest(fd,10,FCGI_RESPONDER,FCGI_KEEP_CONN);
	sendStdin(fd,10,data,strlen(data));
	sendData(fd,10,data,strlen(data));
}
